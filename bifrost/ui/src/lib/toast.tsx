...
const _toastStyles: Record<ToastType, string> = {
...
const _iconStyles: Record<ToastType, string> = {
  success: "✓",
  error: "✕",
  info: "ℹ",
  warning: "⚠",
...
const ToastViewport = ({ removeToast }: ToastViewportProps): ReactElement => {
        ...
        {managedToasts.toasts.map((toast) => {
          ...
          return (
            <BaseToast.Root
              key={toast.id}
              toast={toast}
              className={`border-l-4 p-4 rounded shadow-lg min-w-[300px] max-w-[400px] ${_toastStyles[type]}`}
            >
              <BaseToast.Content className="flex items-start gap-3">
                <span className="text-lg">{_iconStyles[type]}</span>
