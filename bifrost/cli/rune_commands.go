...
func RegisterRuneCommands(root *RootCmd, out *bytes.Buffer) {
	...
	root.Command.AddCommand(NewFailCmd(clientFn, out).Command)
	root.Command.AddCommand(NewReopenCmd(clientFn, out).Command)
	...
	root.Command.AddCommand(NewStateCmd(clientFn, out).Command)
	root.Command.AddCommand(NewRetroCmd(clientFn, out).Command)
