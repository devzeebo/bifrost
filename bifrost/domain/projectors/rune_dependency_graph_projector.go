...
// RuneDependencyGraphTable is the typed table reference for this projector.
// Import and use this from other projectors to get compile-time safety.
var RuneDependencyGraphTable = core.TableRef[RuneDependencyGraphEntry]{Name: "rune_dependency_graph"}
...
func (p *RuneDependencyGraphProjector) Name() string {
	return RuneDependencyGraphTable.Name
}
...
func (p *RuneDependencyGraphProjector) getOrCreateEntry(ctx context.Context, realmID, runeID string, store core.ProjectionStore) (RuneDependencyGraphEntry, error) {
	entry, err := core.GetRef(ctx, store, realmID, RuneDependencyGraphTable, runeID)
	if err != nil {
...
func (p *RuneDependencyGraphProjector) handleAdded(ctx context.Context, event core.Event, store core.ProjectionStore) error {
	...
	for _, dep := range sourceEntry.Dependencies {
		if dep.TargetID == data.TargetID && dep.Relationship == data.Relationship {
			...
			if !dependentExists {
				...
				if err := core.PutRef(ctx, store, event.RealmID, RuneDependencyGraphTable, data.TargetID, targetEntry); err != nil {
					return err
				}
			}
			return nil
	...
	if err := core.PutRef(ctx, store, event.RealmID, RuneDependencyGraphTable, data.RuneID, sourceEntry); err != nil {
		return err
	}
...
func (p *RuneDependencyGraphProjector) handleShattered(ctx context.Context, event core.Event, store core.ProjectionStore) error {
	...
	for _, dep := range entry.Dependencies {
		...
		if err := core.PutRef(ctx, store, event.RealmID, RuneDependencyGraphTable, dep.TargetID, targetEntry); err != nil {
			return err
		}
	}
	...
	for _, dep := range entry.Dependents {
		...
		sourceEntry.Dependencies = filtered
		if err := core.PutRef(ctx, store, event.RealmID, RuneDependencyGraphTable, dep.SourceID, sourceEntry); err != nil {
			return err
		}
	}

	// Delete the shattered rune's own graph entry
	return core.DeleteRef(ctx, store, event.RealmID, RuneDependencyGraphTable, data.ID)
}
