# Bifrost Task Source Plugin PRD v2
...
And a valid PAT exists in ~/.bifrost-credentials for the realm
When the orchestrator loads configuration
Then a BifrostTaskSource is created with the specified settings
And the plugin loads credentials from ~/.bifrost-credentials
...
**I want to** continuously poll for available runes in Bifrost via HTTP API  
...
Filtering is performed via API queries, not direct SQL.
...
Token is loaded from `~/.bifrost-credentials`, not from configuration. The plugin reads the credential file and extracts the PAT for the configured realm.

### FR-7: Credential Loading

The plugin MUST load credentials from Bifrost's credential store:

- Read `~/.bifrost-credentials` file
- Parse credential format (JSON or as defined by Bifrost CLI)
- Extract PAT for the configured realm
- Use PAT as Bearer token for HTTP requests
- Clear error if credentials file is missing or realm not found
...
- Use short-polling only (no long-polling in v2)
...
- Tests MUST verify credential loading from ~/.bifrost-credentials
...
**List Available Runes**
- Endpoint: `GET /api/runes?status=open`
- Purpose: Discover runes ready for agent execution
- Filters applied: No blocking dependencies unsatisfied, not already claimed
- Returns: Array of rune summary objects
...
Per Orchestrator "Out of Scope" section and Bifrost Task Source v2 scope:

The following features are explicitly out of scope for v2:
...
- Direct database access (all access via HTTP API per Feedback FB3)
...
2. Valid PAT exists in ~/.bifrost-credentials for the configured realm
...
15. File system permissions: Orchestrator has read access to ~/.bifrost-credentials
...
- Short-polling with exponential backoff (1s base, 30s max). No long-polling, no webhooks in v2.
...
This section records feedback received after v2 completion for incorporation into v3.

**FB1: Credential File Location and Format (2026-05-08)**
- Current: PRD states credentials are at `~/.bifrost-credentials`
- Correction: Credentials are stored at `~/.config/bifrost/credentials.yaml`
- Format: YAML with `credentials` map keyed by normalized URL
- See: `bifrost/cli/credentials.go` for implementation
- Action for v3: Update FR-7 Credential Loading and all references to credential path

**FB2: Ready Endpoint Usage (2026-05-08)**
- Current: PRD suggests querying `/api/runes` with filters
- Correction: Use the "ready" query pattern from `bf ready` command
- Query: `GET /api/runes?status=open&blocked=false&is_saga=false`
- Rationale: Bifrost has business logic for determining "ready" runes that shouldn't be recreated in the plugin
- Action for v3: Update polling to use ready query pattern, not custom filtering logic
