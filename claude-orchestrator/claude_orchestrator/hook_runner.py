...
This is the anti-corruption layer between shell exit-code conventions and the domain.
The hook runner never calls the SDK client; the orchestrator coordinates follow-ups.
"""
...
from claude_orchestrator.agent_catalog.types import HookSpec
...
class HookRunner:
    ...
    def run_rune_start(
        ...
        """
        rune_json = json.dumps(_rune_to_dict(context.rune))
        parts: list[str] = []

        for hook in hooks:
            try:
                result = self._run_command(hook.command, rune_json, None)
    ...
    def run_rune_stop_once(
        ...
        for hook in hooks:
            try:
                result = self._run_command(hook.command, rune_json, last_agent_message)
            except Exception as exc:
...
def _rune_to_dict(rune) -> dict:
    """Convert a Rune dataclass to a plain dict for JSON serialization."""
    import dataclasses
