...
func TestCheckPATStatus(t *testing.T) {
	...
	t.Run("active PAT", func(t *testing.T) {
		...
		store.data[compositeKey("_admin", "pat_by_id", "pat-123")] = projectors.PATIDEntry{
			PATID:     "pat-123",
			KeyHash:   "keyhash-abc",
			AccountID: "account-456",
		}
		store.data[compositeKey("_admin", "account_auth", "account-456")] = projectors.AccountAuthEntry{
			AccountID: "account-456",
			Username:  "testuser",
			Status:    "active",
	...
	t.Run("revoked PAT - entry deleted", func(t *testing.T) {
		store := newMockProjectionStore()
		// PAT entry doesn't exist (deleted on revocation)
...
func TestValidatePAT(t *testing.T) {
	...
	t.Run("valid PAT", func(t *testing.T) {
		...
		store.data[compositeKey("_admin", "pat_by_id", "pat-789")] = projectors.PATIDEntry{
			PATID:     "pat-789",
			KeyHash:   keyHash,
			AccountID: "account-456",
		}
...
func TestBuildAvailableRealms(t *testing.T) {
	...
	t.Run("regular user sees only their realms", func(t *testing.T) {
		store := newMockProjectionStore()
		store.listData["realm_directory"] = []json.RawMessage{
...
func (m *mockProjectionStore) Get(ctx context.Context, realm, projection, key string, dest interface{}) error {
	...
	case *projectors.RuneSummary:
		...
		*d = e
	case *projectors.RealmDirectoryEntry:
	...
	case *projectors.SystemStatusEntry:
		...
		if !ok {
			return fmt.Errorf("mockProjectionStore.Get: type assertion failed for key %s: expected SystemStatusEntry, got %T", ckey, val)
		}
		*d = e
	default:
